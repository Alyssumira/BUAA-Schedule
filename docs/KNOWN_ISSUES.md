...
- **组件这条链的口径（T46 立实测、T47 加第二档）**：每轮刷新实测一次，图源分两档 ——
  ...
  **但本卡没有那种设备，未实测，不当已验证写**）与主色档（`getWallpaperColors(FLAG_SYSTEM)`，
  零权限，本机唯一真正常开的一档；这枚方法是 API 27 才加的，minSdk 26 那一档机器上这一问根本
  不存在，探针交 null 走第三档 —— 这一道 27 的判断是方法可用性，不是本节日后反复强调的那类
  "拿 API 档次代答设备事实"，两者不许混为一谈）。配置页「这台设备实测读不到系统桌面壁纸」那句**仍然成立、
