...
两套规则都排除 `buaa_cookie_store.xml`（Cookie 密文）与 `buaa_session.xml`
（会话保留标志）。Cookie 密文离开本机后也无法解密（密钥在设备安全硬件里），
但把会话材料复制到别的设备没有任何正当用途，所以直接不迁移。
课表数据库**不**排除 —— 否则换机丢课表。
...
| 权限 | 用途 | 申请时机 |
| --- | --- | --- |
| `INTERNET` | 北航教务登录与课表抓取 | 普通权限，安装即生效 |
