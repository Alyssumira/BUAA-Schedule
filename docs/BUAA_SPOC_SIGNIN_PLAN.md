...
> 缺库时它一样解不出东西，那一页真正剩下的只有手输签到码（⚠️ 这一句的后半截已过时，
> 手输那条入口后来整条拆除，见下面的 T44/T45/T59 订正 ①）。

> **T44/T45/T59 订正（这份计划里三句话与今天的代码不符，别再照它实现）**：
> ① 「手输签到码」那条兜底**整条已删**（T45）—— 现实里老师端只有那张二维码，不存在可以
> 抄下来的短码。所以上面那句"不可用时降级为相册 + 手输"只剩相册一条；**缺库那一档现在是
> 两条一起没有**，降级文案里连"改用相册"都不许出现（`BarhopperNativeLibProbeTest` ⑦ 钉着）。
> ② `signin` scheme 的 intent-filter **从未开**（见 §11 的偏差记录），课前提醒那颗
> 「扫码签到」按钮走的是应用内路由，不是深链。
> ③ 「扫码成功即 `close` 分析流（防重复提交）」**不是实现口径**：分析流一直绑着，防重复
> 提交靠 `ScanSubmissionGate` 的「原文 + 时刻」判据（T44 换掉了那颗 `consumed` 布尔死闸），
> 帧级节奏由 `STRATEGY_KEEP_ONLY_LATEST` + 单线程 executor 压。⚠️ 这一族 bug 的**共同形状**
> 是把一个瞬时状态写成不可恢复的事实：`consumed`（T44）、`scannerWorking` 只写 false（T59①）、
> 绑定失败后不再重跑（T59②）。判据与三道界在 `ui/signin/ScanRecoveryPolicy.kt`，
> 逐条账见 `docs/STATUS.md` 的 T59 那一节。
...
| §5「三条入口：相机实时解码 / 相册识图 / **手输签到码**」（含 §1.2、§5 第 3 步里把手输当兜底的说法） | **手输签到码入口整条删除**（2026-09-21）：底部那颗按钮、输入弹窗、`showManualInput`/`manualCode` 两枚 state 全部拆掉，指向它的降级文案逐条改口径 | 产品拍板：现实里老师端只有那张二维码，**不存在一个可以抄下来的签到码**，这条入口是按假想需求做的。删掉后新增一个此前被手输盖住的事实：相机与相册共用同一颗 scanner，**缺解码库的设备上这一页完全不可用扫码签到**（两条路一起没，一条出路都不剩），`scanUiStatus` 的缺库那一档据此只说实话、不指向任何出路 |
