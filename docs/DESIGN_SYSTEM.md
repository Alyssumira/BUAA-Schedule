...
- **学分：`null` 是"不知道"，`0.0` 是"教务明说这门课不计学分"**，两个值在统计页是两个说法，
  ...
  **显示侧同一条分界**：`null` 时那一行/那一段**整个缺席**（连分隔符一起），不许退化成"未设置"
  占位——占位是给教师、地点那种"总会有但没登记"的字段准备的，学分很可能就是没有；
  `0.0` 要老实画"0学分"。数字一律走 `CourseMetaFormat.formatCredit` 手拼（整数不带小数点、
  小数只留有效位），**不要** `DecimalFormat`/`NumberFormat`：那两位跟 locale 走，
  换台机器"2.5学分"能变成"2,5学分"。
