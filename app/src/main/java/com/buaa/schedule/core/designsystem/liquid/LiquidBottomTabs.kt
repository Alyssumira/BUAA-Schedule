...
fun LiquidBottomTabs(
    ...
    // 0.60 之下这条栏在亮壁纸块上只压出 0.172 的复合亮度，而 onSurfaceVariant（深色档
    // #C5C6D0，亮度 0.5681）压在上面实测 2.78:1 —— ai/T25 报回的就是这一格。
    // 抬天花板能治它，但那条栏悬浮在整屏滚动的课表之上，抬到 AA 需要的 ≈0.74 就是把它
 ...
 * **0.1722**，而 `onSurfaceVariant` `#C5C6D0`（亮度 0.5681）压在上面只有 **2.78:1**
 * （正文要求 [DesignTokens.WCAG_AA_RATIO] = 4.5:1；线性口径的 0.405 / 1.36:1 是一块
 * 真机画不出来的板，见 [com.buaa.schedule.core.designsystem.compositeLuma]）。同族的第二处
 * 修法是把天花板抬到 ≈0.74 —— 那等于把一条悬浮在整屏滚动课表之上的 overlay 压成实心横带，
 * 产品明确不要这个观感（"小玻璃好看、大玻璃板丑"）。所以这一族的这一格改的是**墨**：文字/
 * 图标的墨按实际画出来有多亮解，而不是按主题写死。换完之后同一格是**纯白那支的 4.72:1**
 * （这块真板上近黑只有 3.64:1）。
 *
 ...
 * 内置渐变场景下深色档 5.70:1、浅色档 5.48:1（旧线性口径给 5.35:1 与 5.58:1），
 * 现网绝大多数场景走的就是这条保留分支。
 *
 * ## 黑白都读不清的那一档
 *
