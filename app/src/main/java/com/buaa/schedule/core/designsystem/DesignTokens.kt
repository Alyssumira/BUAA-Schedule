...
object DesignTokens {
     ...
     *
     * 1.40dp/分钟这一档是**按实测行高算出来的**（T61 立的行预算内核 + T61b① 把喂给它的行高
     * 从排版表换成真测量）。一块正常的 45 分钟课要同时给出三行：
     * 课名(labelLarge) / 时间段·节次·教师(labelMedium) / 教室(labelMedium)。装机逐行量到的行框
     * （buaa36 镜像，1080×2400 / 420dpi / 密度 2.625 / fontScale 1.0，色块 Text 节点的实排 bounds）：
     * **52px = 19.81dp** 与 **45px = 17.14dp** ×2，三行合计 142px = **54.10dp**；
     * 而排版表标称是 20 + 16 + 16 = 52dp —— 少算 2.1dp，因为 Compose 的 lineHeight 只是**下限**，
     * 12sp 这档在系统字体下自带 17.14dp 的行框。按标称记账就是 T61 头一版被打回的原因：
     * 它给 1.35 那档算出 54.75dp 容量对 52dp 支出，真画下来只剩 1px（0.38dp）余量，
     * 换一枚 MiSans（HyperOS 自带，垂直度量与 Roboto 不同）就穿，多出来的那一行去压色板自己的描边与圆角。
     *
     * 于是这一档按实测重挑：45 分钟要装下 54.10dp 文字 + 上下各 3dp 内边距 = 60.10dp，
     * 再要 ≥2dp 的真余量（给没量过的字体留的），即 ≥62.10dp ⇒ ≥1.38dp/分钟 ⇒
     * 0.05 一档的最小值 **1.40**：45×1.40 = 63dp，装机复量到的是色板 165px、文字底 1755、
     * 内容上限 1762 ⇒ **真余量 7.1px = 2.71dp**，三行都在、一个字没裁。
     * 旧值 1.05（45 分钟 47.25dp）连第三行的门槛都够不着，教室在单节课上永远不出现——
     * 用户报的「时间轴模式显示的文字内容有点少」就是那一档差出来的。
     * 哪些行装得下由 DayTimelineBlockLines.kt 逐块算，这里不再留魔法数。
     *
     * 代价是滚动长度，同样按真机量过、不按排版表推：种子课表的日窗口是整点吸附后的
     * 08:00–23:00 共 900 分钟，1.35 档 3189px → **1.40 档 3307.5px**（=1260dp，装机 log 的
     * totalPx 原值），多滚 118px ≈ 45dp；真机上这一屏的正文视口约 1104px（420dp），
     * 也就是多半屏都不到。大字号那一档不归这条算式管：fontScale 2.0 时实测单行 99px，
     * 45 分钟的块只剩课名（内核逐块裁），那是设计不是事故。
     * 触控下限 [minTouchTarget] 不随之动：块高整体放大，被下限补齐吃掉的溢出量反而变小
     * （48dp 现在只相当于 34.3 分钟，1.35 档是 35.6 分钟、1.05 档曾是 45.7 分钟），
     * 相邻两节互相压住的空间是一路收窄的。
     */
    val dayHeightPerMinute = 1.40.dp
