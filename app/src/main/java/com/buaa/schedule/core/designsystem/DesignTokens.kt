...
object DesignTokens {
     ...
     *
     * 1.35dp/分钟这一档是**按文字算出来的**（T61）：一块正常的 45 分钟课，要在
     * 「课名 labelLarge 20sp + 时间段/教师 16sp + 教室 16sp + 上下各 3dp 内边距」
     * 之下不裁切，至少需要 58dp；旧值 1.05 只有 47.25dp（补齐到 48dp 也还不够），
     * 于是教室那一行在单节课上永远不出现——用户报的「时间轴模式显示的文字内容有点少」
     * 就是这一档差出来的。抬到 1.35 以后 45×1.35 = 60.75dp ≥ 58dp，
     * 单节课第一次能同时给出课名/时间/教室，连排的块（95、155 分钟）再按同一份行预算
     * 长出备注行；哪些行装得下由 DayTimelineBlockLines.kt 算，这里不再留魔法数。
     *
     * 代价是滚动长度，这一条按真机量过、不按排版表推（buaa36 / Pixel 7 / API 36，
     * 密度实测 2.625px/dp）：日视图的窗口是整点吸附后的首末节次跨度，种子课表上那是
     * 07:00–22:00 共 900 分钟，改前 945dp（2481px）、改后 1215dp（3189px），
     * 比值 1.286 恰为 1.35/1.05 —— 多滚约 270dp，而真机上时间轴的正文视口只有 480dp
     * （1260px，Hero 摘要卡之下那一截），也就是多半屏的路。
     * 触控下限 [minTouchTarget] 不随之动：块高整体放大，被下限补齐吃掉的溢出量反而变小
     * （48dp 现在只相当于 35.6 分钟，此前是 45.7 分钟），相邻两节互相压住的空间是收窄的。
     */
    val dayHeightPerMinute = 1.35.dp
