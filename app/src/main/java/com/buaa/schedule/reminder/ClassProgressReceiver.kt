...
class ClassProgressReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        // 上/下课铃这串副作用是 5 次以上 binder 连发（AlarmManager 排程、notify、
        // startForegroundService、setInterruptionFilter、prefs 落盘），闹钟叫醒的冷进程里
        // 全落在广播主线程上就是在吃 10 秒配额，超配额系统直接掐广播 ——
        // "这一节课没有提醒/勿扰没恢复"就是这么来的（判据同 ReminderReceiver，审计 §2.6）。
        // 主线程只留两件事：领 PendingResult、把主体交给协程。
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // 挪进协程后这道保护不会自动保留：协程里冒到 launch 之外的异常走的是默认
                // uncaught handler，效果一样是杀进程，所以 runCatching 得包在唤醒锁外面。
    ...
    /**
     * 下课之后续排下一个课堂窗口（要查库，所以整条 ACTION_END 都不能留在广播主线程上）。
     *
     * 对关掉了课前提醒的用户，这是唯一的再排程入口 ——
     * [ReminderScheduler.rescheduleAll] 由课前提醒闹钟驱动，提醒全关时它不排课堂窗口。
     * 少了这一步，实况与上课自动勿扰在第一节课之后就再也不会出现。
     */
    private suspend fun rescheduleNextWindow(context: Context) {
        // 这里不再自己 goAsync：一条广播只有一个 PendingResult，第二次调用返回的是 null，
        // finish() 当场 NPE、续排根本不会跑。改由 onReceive 那一份负责"广播别被提前掐"，
        // 这一步在协程里被 await，PendingResult 就活到它跑完。
        // 单独吞异常是为了后面那句 requestLiveRefresh 不被这一步的失败连坐。
        runCatching {
            // 续排要查三张表再重排闹钟，Doze 下没锁就会中途睡回去，链条断在这里（同 ReminderReceiver）。
            // 这一把锁独立于外层 class_progress：按 tag 数持锁次数是审计 §4.3 的对账口径。
            WakeLocks.withPartialWakeLock(context, "class_reschedule") {
                ClassProgressScheduler.rescheduleNextWindow(context)
            }
        }.onFailure { android.util.Log.w(TAG, "续排下一节课堂窗口失败", it) }
    }
