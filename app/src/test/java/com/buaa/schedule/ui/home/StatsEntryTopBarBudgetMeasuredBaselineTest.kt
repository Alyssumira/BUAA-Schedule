 ...
 * **整块被挤到屏外**。像素证据（PIL 扫第二行 y=300..400 的蓝色字面，校区切换是
 * `CampusPickerButton` 那颗蓝色文字按钮）：
 *  - 今日页签 `T69b-10-pre-day.png`：蓝色字面落在 **x=43..218**（155 个蓝点）⇒ 按钮在场；
 *  - 周课表页签 `T69b-11-pre-week.png`：蓝色只剩 **x=466..518**（53 个蓝点）—— 那是
 *    `SemesterProgressLine` 的进度条，不是校区按钮；x≥860 全段只有 19 个暗点
 *    （x=988..1006，就是 › 那颗箭头的笔画）⇒ 校区按钮一个像素都没画出来。
...
class StatsEntryTopBarBudgetMeasuredBaselineTest {
    ...
    /**
     * 第二档（第二行尾部）实测**没有预算**：周次簇按父级整份宽度（1038px）铺开，
     * `campusSlot` 拿到 maxWidth = 0，那颗 233px 的校区按钮整块在屏外
     * —— 今日页签它画在 x=43..218，周课表页签同一行 x≥860 只剩 › 箭头那 19 个暗点。
