...
private class DrawBackdropNode(
    ...
    private fun updateEffects(geometryChanged: Boolean = false) {
        ...
        // Attach 早于首次 draw：effectScope 尚无几何信息，跳过求值，等 draw 时重建。
        // （Compose 1.7 的 Size.minDimension 对 Unspecified 直接抛异常）
    ...
    fun synchronizeSampleLayer() {
        ...
        if (shapeProvider.options.sampleBackdrop) {
            if (graphicsLayer == null) {
                graphicsLayer = BackdropGraphicsLayerPool.acquire(context)
                lastEffectKey = null
            ...
            graphicsLayer?.let {
                BackdropGraphicsLayerPool.release(context, it)
    ...
    override fun onAttach() {
        BackdropSurfaceRegistry.acquire()
        BackdropDiagnostics.event("Surface.Attach active=${BackdropSurfaceRegistry.activeCount}")
    ...
    override fun onDetach() {
        ...
        BackdropSurfaceRegistry.release()
        BackdropDiagnostics.event("Surface.Detach active=${BackdropSurfaceRegistry.activeCount}")
        graphicsLayer?.let { layer ->
            BackdropGraphicsLayerPool.release(graphicsContext, layer)
            layerDiagnostics.released()
            graphicsLayer = null
        }
