...
桌面组件那一层不是这样 —— 它每轮刷新实测一次，实测读不到就退成一层纯色半透明底，不会拿你自选的图去顶。
见 [`docs/KNOWN_ISSUES.md`](docs/KNOWN_ISSUES.md) §1）。
...
的读取 —— 后者在仪器测试里没有断言，而"Android 14 起读不到桌面壁纸"这个前提也已被组件那条链
的实测证伪（见 [`docs/KNOWN_ISSUES.md`](docs/KNOWN_ISSUES.md) §1）。
